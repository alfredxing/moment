var prefs=Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.moment.");var background="#FFF",text="#000",degrees="",temp="c",clock="24",expires="",weather="",tform="HMS",dform="D M Y",loc=":auto",interval;
...
refresh();getWeather();}function setExpires(b){prefs.setCharPref("expires",b);}function setDegrees(b){prefs.setCharPref("degrees",b);}function setWeather(b){prefs.setCharPref("weather",b);
}function request(){$.ajax({url:"https://api.aerisapi.com/observations/closest?p="+loc+"&client_id=wgE96YE3scTQLKjnqiMsv&client_secret=sFA4Gfz8s8brxUUHoZTQ3iZIRy6YSwLhf1wJQzDi",dataType:"jsonp",success:function(a){var c=a.response[0].ob.weather;
...
storeWeather(b,c);}});}function refresh(){var h=new Date();var c=h.getHours();var b=h.getMinutes();var j=h.getSeconds();if(clock=="12"&&c>12){c=24-c;}if(10>c){c="0"+c;
...
$("#conditions").animate({opacity:1},300);}else{request();}}function load(){recall();interval=setInterval(function(){refresh();},1000);$("body").animate({opacity:1},500);
}function install_notice(){if(localStorage.getItem("install_time")){return;}var a=new Date().getTime();localStorage.setItem("install_time",a);chrome.tabs.create({url:"options.html"});
}$(document).ready(function(){refresh();load();});